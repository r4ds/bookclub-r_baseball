...
- Review fitting models with R
- Study player career trajectories

## Mickey Mantle's trajectory
...
- Uses a measure of simularity developed by Bill James, using both:
    - Position
...
- Value_POS are numbers introduced for use in the simularity score by Bill James for positions.

## Compute Simularity Score {-}

- Function finds `number` simular players to a given player based on Bill Jame's simularity
